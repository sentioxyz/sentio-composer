...
use crate::helper::{absolute_path, get_function_abi, get_node_url};
...
fn main() {
    ...
    exec_func(func, type_params, params, ledger_version, network, &mut execution_result);
...
fn exec_func(func: String, type_params: String, params: String, ledger_version: u64, network: String, execution_res: &mut ExecutionResult) {
    let client = Client::new(get_node_url(network.clone()));
    // TODO(pcxu): serialize params according to abi
    // let abi = get_function_abi(client.to_owned());
    ...
    match account {
        Ok(addr) => {
            ...
            let storage = InMemoryLazyStorage::new(ledger_version, network, client);
            let res = exec_func_internal(storage, module, IdentStr::new(splitted_func.next().unwrap()).unwrap(), type_args, args);
            match res {
                None => {
                    println!()
                }
...
fn exec_func_internal(storage: InMemoryLazyStorage, module: ModuleId, function: &IdentStr, type_args: Vec<TypeTag>, args: Vec<Vec<u8>>) -> Option<Vec<(Vec<u8>, MoveTypeLayout)>> {
...
mod tests {
    ...
    use crate::{exec_func_internal, get_node_url, InMemoryLazyStorage};

    static MAINNET: Lazy<String> = Lazy::new(|| {
        String::from("mainnet")
    });

    static TESTNET: Lazy<String> = Lazy::new(|| {
        String::from("testnet")
    });
    ...
    fn test_call_aptos_function_vault() {
        SimpleLogger::init(LevelFilter::Info, Config::default()).unwrap();
        let client = Client::new(get_node_url(TESTNET.to_owned()));
        let storage = InMemoryLazyStorage::new(334551341, TESTNET.to_owned(), client);
