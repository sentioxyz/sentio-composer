...
impl CacheModuleResolver {
    ...
    fn try_load_module_from_disk_cache(
        ...
        let module_cache_key = self.get_cache_key(module_address.as_str(), module_name);
    ...
    fn cache_module_to_disk(&self, module_address: &str, module_name: &str, bytecode: Vec<u8>) {
        debug!("Caching {}::{} to disk", module_address, module_name);
        cacache::write_sync(
            self.get_cache_path(self.cache_folder.clone()),
            self.get_cache_key(module_address, module_name),
            bytecode,
        )
        .expect("Failed to cache the module");
    }

    fn get_cache_key(&self, module_address: &str, module_name: &str) -> String {
        let mut module_cache_key = module_address.to_string();
